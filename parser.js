    ...
    for (var i=0; i<threads.length; i++)
    {
      ...
      {        
        ...
        else if (sub.search("Portal edit submission confirmation") != -1)
        {
          ...
          portalEditSubmission(sub, dat, t, label, doneLabel, whoTo, "", "", whoTo, "");
          //portalEditSubmission(subjectLine, date, theThread, label1, label2, locationString, newDesc, whoTo, base64)
        }
        else if (sub.search("Portal Edit Suggestion") != -1)
        {
          ...
          {
            portalEditSubmission(msgArray[14], dat, t, label, doneLabel, "None", "None", whoTo, "0");
          } else {
            var combined = msgHTMLSplit[123] + msgHTMLSplit[124];
            portalEditSubmission(msgHTMLSplit[108], dat, t, label, doneLabel, msgHTMLSplit[125],combined, whoTo, msgArray[25]);
          }
        }
        else if (sub.search("Portal edit review complete") != -1)
        {
          ...
          if( len == 1 ) // we have a REDACTED email.
          {
            ...
            portalEditReviewComp(msgHTMLSplit[108], msgHTMLSplit[111], dat, t, label, doneLabel, whoTo, combined, msgHTMLSplit[124], msgArray[23]);
          }
        }
        else if (sub.search("Portal photo review complete") != -1)
        {
          ...
          {
            // redacted
            // function photoParser(subjectLine, date, theThread, label1, label2, title, img, whoTo, body, id)
            photoParser(sub, dat, t, label, doneLabel, msgArray[6], "None", whoTo, "None", "ID: 0");
          }
          else
          {
            photoParser(sub, dat, t, label, doneLabel, msgHTMLSplit[108], msgHTMLSplit[120], whoTo, msgHTMLSplit[112], msgArray[21]);
          }
        }
        else if (sub.search("Portal photo submission") != -1)
        {
          ...
          if( len == 1 )
          {
            ...
            photoSubParser(msgArray[6], dat, t, label, doneLabel, whoTo,"None", "ID: 0");
          } else {
            photoSubParser(msgHTMLSplit[108], dat, t, label, doneLabel, whoTo,msgHTMLSplit[123], msgArray[23]);
          }
        }
        ...
        {
          invalid_portal_parser(sub,msgHTMLSplit[108],msgHTMLSplit[111],dat, t, label, doneLabel, whoTo, msgArray[19]);
        }        
        else if ( (sub.search("Nominating") != -1) )
        {
          poke_submission_parser(dat, msgArray[10], msgArray[12], msgHTMLSplit[164].replace(/<\/?[^>]+(>|$)/g, ""), t, label, doneLabel, whoTo, msgArray[16]);
        }
        ...
        else if( sub.search("Invalid Pokéstop/Gym Report Received") != -1)
        {
          Logger.log("INV");
          //function poke_invalid_rejected( date, title, theThread, label1, label2, whoTo, base64Id)
          poke_invalid_rx( dat, msgArray[8], t, label, doneLabel, whoTo, msgArray[16]);
        }
        else if( sub.search("Invalid Pokéstop/Gym Report Rejected") != 1)
        {
          //function poke_invalid_rejected( date, title, theThread, label1, label2, whoTo, base64Id
          Logger.log("INV");
          // id 16
          // name 8
          poke_invalid_rejected(dat, msgArray[8], t, label, doneLabel, whoTo, msgArray[16]);
        }
      }
    }
  }
...
** @brief 
**************************************************************************************/
function poke_submission_parser(date, title, desc, img, theThread, label1, label2, whoTo, base64Id)
{
  var length = img.length;
  var imgUrl = img;
  var t = imgUrl.replace(' ', '');
  var ID = getBase64Id(base64Id);
  if(findInRow(date) == -1)
  { 
    addToSubmittedRow("SUBMITTED", date, title, whoTo, ID);
...
{
  var Name = getName(title);
  var ID = getBase64Id(base64Id);
  var rowIndex = findEditedEntry("NULL", Name, ID);
  if(rowIndex == -1)
  {
    var discordStr = "Portal Edit Submitted - " + Name + "\nNew Desc/Title: " + sugg;
    addToEditedRow("EDITED", date, Name, whoTo, ID);
...
{
  var s = title.indexOf(":");
  var Name = title.substr(s+2,title.length);
  return Name;
}
...
** @brief 
**************************************************************************************/
function getBase64Id(base64Id)
{
  var t = base64Id.indexOf(":");
  var ID = base64Id.substr(t+2,base64Id.length);
  return ID;
}
...
** @brief 
**************************************************************************************/
function getBase64EncodedString( inStr )
{
  // Instantiates a blob here for clarity
  var blob = Utilities.newBlob(inStr);
  // Writes 'QSBzdHJpbmcgaGVyZQ==' to the log.
  var encoded = Utilities.base64Encode(blob.getBytes());
  //Logger.log("getBase64EncodedString: " + encoded);
  return encoded;
}
...
** @brief 
**************************************************************************************/
function poke_invalid_rx( date, title, theThread, label1, label2, whoTo, base64Id)
{
  var Name = getName(title);
  var ID = getBase64Id(base64Id);
  var rowIndex = findInvalidEntry("NULL", Name);
  
  if(rowIndex == -1)
  {
    var discordStr = "Invalid Portal Submitted - " + Name;
    addToInvalidRow("INVALID", date, Name, whoTo, "SUBMITTED", ID);
    postMessageToDiscord(discordStr, "None", whoTo);
    move_thread( theThread, label1, label2 );
  }
  else
  {
    move_thread( theThread, label1, label2 );
    Logger.log("poke_invalid_rx: Name already found in Invalid: " + Name);
  }
}
...
function poke_invalid_rejected( date, title, theThread, label1, label2, whoTo, base64Id)
{
  ...
  if(rowIndex != -1)
  {
    var discordStr = "Invalid Portal Rejected - " + Name;
  ...
  {
    move_thread( theThread, label1, label2 );
    Logger.log("poke_invalid_rejected: Name already found in Invalid: " + Name);
  }
}
...
** @brief 
**************************************************************************************/
function photoSubParser(name, date, theThread, label1, label2, whoTo, img, base64Id)
{
  var ID = getBase64Id(base64Id);
  var PortalName = name;
  var rowIndex = findPhotoEntry("NULL", PortalName, ID);
  if(rowIndex == -1)
  {
    addToPhotoRow("PHOTO", date, PortalName, whoTo, "SUBMITTED", ID);
    var s = img.indexOf("<");
...
** @brief 
**************************************************************************************/
function invalid_portal_parser(subject, name, bodyText, date, theThread, label1, label2, whoTo, base64Id)
{
  var ID = getBase64Id(base64Id);
  var PortalName = name;
  ...
  if(subject.search("reviewed") != -1)
  {
    if( bodyText.search("remain") != -1)
    {
      ...
      Logger.log("invalid_portal_parser: 0 Finding Entry: " + PortalName + " at: " + rowIndex);
      if( rowIndex != -1)
      {        
    ...
    {
      var rowIndex = findInvalidEntry("NULL", PortalName);
      Logger.log("invalid_portal_parser: 1 Finding Entry: " + PortalName + " at: " + rowIndex);
      if( rowIndex != -1)
      {        
      ...
      {
        //addToAcceptedRow("INVALID", date, PortalName, whoTo);
        addToInvalidRow("INVALID", date, PortalName, whoTo, "SUBMITTED", ID);
        rowIndex = findInvalidEntry("NULL", PortalName);
  ...
  else
  {
    ...
    Logger.log("invalid_portal_parser: Finding Entry: " + PortalName + " at: " + date);
    if(ind == -1)
      {        
        addToInvalidRow("INVALID", date, PortalName, whoTo, "SUBMITTED", ID);
...
{
  var len = subjectStr.length;
  var PortalName = subjectStr.substr(24,len);
  var baseId = getBase64EncodedString(PortalName);
  var imageUrl = get_portal_bot_response(PortalName);
  if ( (bodyText.search("rejected") == -1) && (bodyText.search("rejection") == -1) )
  {
    if ( (bodyText.search("too close") == -1) && (bodyText.search("online at this time") == -1) )
    {
      if(findInRow(date) == -1)
      {        
        var rowIndex = findNominationEntry("NULL", "NULL", baseId);
        addToAcceptedRow("ACCEPTED", date, PortalName, whoTo);
        if( rowIndex == -1 ) {
          Logger.log("portalReviewComplete: Could not find in Nominations rows... adding");
          addToNominationsRow("SUBMISSION", date, PortalName, whoTo, "SUBMITTED", baseId);
          Logger.log("portalReviewComplete: Finding new Row Number....");
          rowIndex = findNominationEntry("NULL", "NULL", baseId);
          Logger.log("portalReviewComplete: Found at " + rowIndex);
          if( rowIndex != -1 ){
            modifyNominationsRow(rowIndex, "ACCEPTED", date);
          } else {
            Logger.log("portalReviewComplete: Something is wrong...");
          }
        } else {
          modifyNominationsRow(rowIndex, "ACCEPTED", date);
        }
    ...
    else
    {
      if(findInRow(date) == -1)
      {
        var rowIndex = findNominationEntry("NULL", "NULL", baseId);
        if( rowIndex == -1 ) {
          Logger.log("portalReviewComplete: Too Close: Could not find in Nominations rows... adding");
          addToNominationsRow("SUBMISSION", date, PortalName, whoTo, "SUBMITTED", baseId);
          Logger.log("portalReviewComplete: Finding new Row Number....");
          rowIndex = findNominationEntry("NULL", "NULL", baseId );
          Logger.log("portalReviewComplete: Found at " + rowIndex);
          if( rowIndex != -1 ){
            modifyNominationsRow(rowIndex, "DUP/CLOSE", date);
          } else {
            Logger.log("portalReviewComplete: Something is wrong...");
          }
        } else {
    ...
    if(findInRow(date) == -1)
    {
      if( bodyText.search("rejected due to the") != -1)
      {
        ...
        if( rowIndex == -1 ) {
          ...
          Logger.log("portalReviewComplete: Found at " + rowIndex);
          if(rowIndex != -1){
            modifyNominationsRow(rowIndex, "REJECTED", date);
          } else {
            Logger.log("portalReviewComplete: Something is wrong...");
          }
        } else {
...
** @brief 
**************************************************************************************/
function portalEditSubmission(subjectLine, date, theThread, label1, label2, locationString, newDesc, whoTo, base64)
{
  var PortalName = subjectLine;
  var discordStr = "";
  var paren = locationString.indexOf(")"); 
  var portal_photo_link = "";
  var id = getBase64Id(base64);
  if ( paren > -1 )
  {
  ...
  var rowIndex = findEditedEntry(PortalName, date, id);
  Logger.log("portalEditSubmission: Found " + PortalName + " at: " + rowIndex + " " + date + " " + id);
  if(rowIndex == -1)
  {
    Logger.log("portalEditSubmission: Adding " + PortalName + " to Edited Row");
    addToEditedRow("EDITED", date, PortalName, whoTo, id);
    postMessageToDiscord(discordStr, portal_photo_link, whoTo);
    move_thread( theThread, label1, label2 );
  }
  else
  {
    move_thread( theThread, label1, label2 );
    Logger.log("portalEditSubmission: This entry exists! " + PortalName + " id = " + id);
  }
}
...
** @brief 
**************************************************************************************/
function portalEditReviewComp(Name,bodyText, date, theThread, label1, label2, whoTo, thingChanged, locationStr, base64)
{
  var PortalName = Name;
  var portal_photo_link = "";
  Logger.log("portalEditReviewComp: " + PortalName);
  //Logger.log(bodyText);
  var id = getBase64Id(base64);
  var paren = -1;
  if( locationStr != "None" ){
  ...
  {
    var rowIndex = findEditedEntry("NULL", PortalName, id);
    if(rowIndex != -1)
    {
      modifyEditedRow(rowIndex, "ACCEPTED", date)
    ...
    {
      addToEditedRow("EDITED", date, PortalName, whoTo, id);
      Logger.log("portalEditReviewComp: This entry does not exist in Edited, added it: " + PortalName + " id: " + id);
    }
  }
  else if( bodyText.search("decided not to") != -1)
  {
    var rowIndex = findEditedEntry("NULL", "NULL", id); //"NULL", PortalName, id);
    if(rowIndex != -1)
    {
      modifyEditedRow(rowIndex, "REJECTED", date)
...
** @brief 
**************************************************************************************/
function photoParser(subjectLine, date, theThread, label1, label2, title, img, whoTo, body, base64)
{
  var id = getBase64Id(base64);
  ...
  else
  {
    ...
    addToPhotoRow("PHOTO", date, title, whoTo, "SUBMITTED", id);
    move_thread( theThread, label1, label2 );
...
function submissionConfParser(bodyText, subjectLine, date, theThread, label1, label2, imgUrl, whoTo)
{
  var PortalName = "";
  var len = subjectLine.length;
  PortalName = subjectLine.substr(32,len);
  var baseId = getBase64EncodedString(PortalName);
  if(findInRow(date) == -1)
  {
    addToSubmittedRow("SUBMITTED", date, PortalName, whoTo, 0);
    addToNominationsRow("SUBMISSION", date, PortalName, whoTo, "SUBMITTED", baseId);
...
** @brief 
**************************************************************************************/
function addToSubmittedRow(type, date, data, whoTo, id)
{
  var submittedSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Submitted");
  submittedSS.appendRow([type, date, data, whoTo, "SUBMITTED", "","", id]);
}
...
** @brief 
**************************************************************************************/
function addToInvalidRow(type, date, data, whoTo, status, id)
{
  var InvalidSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Invalid");
  InvalidSS.appendRow([type, date, data, whoTo, status, "", "", id]);
}

/**************************************************************************************
** @brief 
**************************************************************************************/
function addToNominationsRow(type, date, data, whoTo, status, id)
{
  var NomSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Nominations");
  NomSS.appendRow([type, date, data, whoTo, status, "", "", id]);
}

/**************************************************************************************
** @brief 
**************************************************************************************/
function addToPhotoRow(type, date, data, whoTo, status, id)
{
  var PhotoSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Photo");
  PhotoSS.appendRow([type, date, data, whoTo, status, "", "", id]);
}
...
** @brief 
**************************************************************************************/
function modifyNominationsRow(rowIndex, newValue, date)
{
  var NomSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Nominations");
  NomSS.getRange(rowIndex, 5 ).setValue(newValue); 
  // Add the new date for the Accept/reject.
  NomSS.getRange(rowIndex, 6 ).setValue(date); 
}
...
** @param name
**************************************************************************************/
function genericRowSearch(rows, date, name, id)
{
  var t = rows.length;
  ...
  { 
    ...
    {
      if ( (rows[r].join("#").indexOf(date) !== -1) && (rows[r].join("#").indexOf(name) !== -1) && (rows[r].join("#").indexOf(id) !== -1)) 
      {
        return r+1;
      }
    } 
    ...
    {
      if ( (rows[r].join("#").indexOf(id) !== -1) && (rows[r].join("#").indexOf(name) !== -1) ) 
      {
         return r+1;
      }
    } 
    ...
    {
      if ( (rows[r].join("#").indexOf(id) !== -1) && (rows[r].join("#").indexOf(date) !== -1) )
      {
         return r+1;
      }
    } 
...
function findSubmittedEntry( date, name )
{
  ...
  return genericRowSearch(subrows, date, name, "NULL");
}

/**************************************************************************************
** @brief Find a Nomination Entry
** @param date The date of the email
** @param name The Name of the Portal
**************************************************************************************/
function findNominationEntry( date, name, id )
{
  var nomSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Nominations");
  var nomrange = nomSS.getDataRange();
  var nomrows  = nomrange.getValues(); 
  return genericRowSearch(nomrows, date, name, id);
}
...
function findAcceptedEntry( date, name )
{
  ...
  return genericRowSearch(accrows, date, name, "NULL");
}
...
function findRejectedEntry( date, name )
{
  ...
  return genericRowSearch(rejrows, date, name, "NULL");
}
...
** @param name The Name of the Portal
**************************************************************************************/
function findEditedEntry( date, name, id )
{
  var EditedSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Edited");
  var edjrange = EditedSS.getDataRange();
  var edjrows  = edjrange.getValues(); 
  return genericRowSearch(edjrows, date, name, id);
}
...
function findInvalidEntry( date, name ){
  ...
  return genericRowSearch(injrows, date, name, "NULL");
}
...
function findMissionsEntry( date, name ){
  ...
  return genericRowSearch(missrows, date, name, "NULL");
}
...
** @param name The Name of the Portal
**************************************************************************************/
function findPhotoEntry( date, name, id){
  var PhotoSS = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Photo");
  var photorange = PhotoSS.getDataRange();
  var photorows  = photorange.getValues(); 
  return genericRowSearch(photorows, date, name, id);
}
...
function findInRow(data) {
  ...
  ret_val = findEditedEntry(data, "NULL", "NULL");
  if ( ret_val !== -1 ) {
    return ret_val;
  }
